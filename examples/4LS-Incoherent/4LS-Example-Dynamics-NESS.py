...
r_g1 = np.sqrt(rs[0, 0] * rs[0, 1])
r_g2 = np.sqrt(rs[1, 0] * rs[1, 1])
r_e1 = np.sqrt(rs[0, 0] * rs[1, 0])
r_e2 = np.sqrt(rs[0, 1] * rs[1, 1])
r_par = np.sqrt(rs[0, 0] * rs[0, 1])
r_X = np.sqrt(rs[1, 0] * rs[1, 1])
...
L_sec[2, 2] = -(gammas[0, 0] + gammas[1, 0]) - (rs[0, 0] + rs[1, 0])
L_sec[3, 3] = -(gammas[0, 1] + gammas[1, 1]) - (rs[0, 1] + rs[1, 1])
# Populations Entering Ground State b/c Emission
# ei -> g1
L_sec[0, 2] = gammas[0, 0] + rs[0, 0]
L_sec[0, 3] = gammas[0, 1] + rs[0, 1]
# ei -> g2
L_sec[1, 2] = gammas[1, 0] + rs[1, 0]
L_sec[1, 3] = gammas[1, 1] + rs[1, 1]
...
L_ns[0, 4] = -(p_e1 * r_e1 + p_e2 * r_e2)
L_ns[1, 4] = -(p_e1 * r_e1 + p_e2 * r_e2)
L_ns[2, 4] = 2 * p_e1 * r_e1
L_ns[3, 4] = 2 * p_e2 * r_e2
# Faster GS coherence loss b/c excitation interference
L_ns[4, 0] = -0.5 * (p_e1 * r_e1 + p_e2 * r_e2)
L_ns[4, 1] = -0.5 * (p_e1 * r_e1 + p_e2 * r_e2)

# Interference in Emission
# Emitting to Coherent Superposition
L_ns[4, 2] = p_e1 * (g_e1 + r_e1)
L_ns[4, 3] = p_e2 * (g_e2 + r_e2)
# Faster Population Transfer
L_ns[2, 6] = - (p_g1 * (g_g1 + r_g1) + p_g2 * (g_g2 + r_g2))
L_ns[3, 6] = - (p_g1 * (g_g1 + r_g1) + p_g2 * (g_g2 + r_g2))
L_ns[0, 6] = 2 * p_g1 * (g_g1 + r_g1)
L_ns[1, 6] = 2 * p_g2 * (g_g2 + r_g2)
# Faster ES coherence loss b/c emission interference
L_ns[6, 2] = -0.5 * (p_g1 * (g_g1 + r_g1) + p_g2 * (g_g2 + r_g2))
L_ns[6, 3] = -0.5 * (p_g1 * (g_g1 + r_g1) + p_g2 * (g_g2+ r_g2))

# Coherence Transfer
# GS -> ES Coherence Transfer via Excitation
L_ns[6, 4] = (p_par * r_par + p_X * r_X)
L_ns[7, 5] = (p_par * r_par - p_X * r_X)
# ES -> GS Coherence Transfer via Emission
L_ns[4, 6] = (p_par * (g_par + r_par) + p_X * (g_X + r_X))
L_ns[5, 7] = (p_par * (g_par + r_par) - p_X * (g_X + r_X))
...
ax[0, 1].axhline(0, c='k',  linestyle=':')
ax[1, 1].axhline(0, c='k', linestyle=':')
